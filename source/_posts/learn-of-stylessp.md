...
1. DDIM 反演得到内容图潜变量 $z_T$
2. 在反演阶段加入负引导，让 $z_T$ 远离风格图内容，得到期望的内容图潜变量 $z_T^c$
3. 对 $z_T^c$ 做频率操作，削弱低频作为采样起点 $z_T^{c,\prime}$
...
- Stable Diffusion 先将图像编码为潜变量 $z$，在潜空间加噪/去噪，再解码回像素。通过将随机噪声向量 $z_t$ 与文本 prompt $P$ 映射到输出图像 $I_0$，并使用交叉注意力，使输出图像与条件 prompt 语义对齐
- 学习目标是使 $Loss$ 最小（去噪回归）：

$$
\begin{aligned}
L &= \mathbb{E}_{z_0,\ \epsilon\sim\mathcal{N}(0,I),\ t\sim\mathrm{Uniform}(1,T)} \bigl\| \epsilon - \epsilon_\theta(z_t, t, C) \bigr\|_2^2
\end{aligned}
$$

- $I$：输入图片；$z_0 = E(I)$：编码后潜变量；$t$：从 ${1,\dots,T}$ 中均匀采样的时间步；$C = \phi(P)$：对 $\phi$ 使用 text encoder 后生成的文本嵌入；$\epsilon$：真实噪声；$\epsilon_\theta$：U-Net 预测噪声。
...
= \epsilon_\theta(z_t, t, \varnothing) + \omega\!\left(\epsilon_\theta(z_t, t, C) - \epsilon_\theta(z_t, t, \varnothing)\right)
$$

- $\omega \ge 0$：guidance scale，用于调节条件预测 $\epsilon_\theta(z_t, t, C)$ 与无条件预测 $\epsilon_\theta(z_t, t, \varnothing)$ 的相对权重。

### Denoising Diffusion Implicit Model（DDIM） Inversion

**去噪扩散隐式模型** 通过非马尔可夫扩散过程提升生成效率与质量，减少生成所需步数。在 SD 中，确定性的 DDIM 采样利用 denoiser 网络 $\epsilon_\theta$，其 **反向采样过程** 用预测噪声一步步还原来反向去噪，更新为：

$$
z_{t-1} = \sqrt{\frac{\alpha_{t-1}}{\alpha_t}}\, z_t
+ \left( \sqrt{\frac{1}{\alpha_{t-1}} - 1} - \sqrt{\frac{1}{\alpha_t} - 1} \right)\epsilon_\theta(z_t, t)
$$

- 其中 $\alpha = (\alpha_1,\dots,\alpha_T)\in\mathbb{R}_{\ge 0}^T$ 是定义 $T$ 个 diffusion 步的噪声强度超参数。本文采用公开的 SD 模型：正向扩散应用在图像编码 $z_0 = E(I_0)$ 上，反向扩散结束后通过解码器获得图像 $I_0 = D(z_0)$

将 DDIM 采样表示成常微分方程，则 **反演过程（正向）** 可写为：

$$
z^*_{t+1}
= \sqrt{\frac{\alpha_{t+1}}{\alpha_t}}\, z^*_t
+ \sqrt{\alpha_{t+1}}\!\left(\sqrt{\frac{1}{\alpha_{t+1}} - 1} - \sqrt{\frac{1}{\alpha_t} - 1}\right)\epsilon_\theta(z^*_t, t)
$$

- 其中 $z^*_t$ 表示 DDIM 反演过程中的潜变量特征。由此可以得到 DDIM 反演轨迹 $[z^*_t]_{t=0}^T$。已有工作表明将 $z_T = z^*_T$ 作为采样起点最有利于原始内容保持。
...
- 在 DDIM 反演前替换噪声预测为带负 prompt 的形式，直接作用于起点：
  ...
  $$
  \hat{\epsilon}_\theta(z_t, t, C_+, C_-)
  = \epsilon_\theta(z_t, t, C_-) + \omega_i\!\left(\epsilon_\theta(z_t, t, C_+) - \epsilon_\theta(z_t, t, C_-)\right)
  $$

- 比采样阶段的负prompt更能避免多条件冲突，减少风格物体泄漏
- 负引导尺度 $\omega_i$：从 1.0 提升到 3.0 可逐步降低风格泄漏，可按偏好调节，默认 1.5

### 频率操作（Frequency Manipulation）

- 已知：高频成分主导轮廓/布局，低频成分影响较小
- 做法：在频域将低频衰减为 $\alpha \in [0,1]$ 倍，同时按 $(1-\alpha)$ 比例注入高斯噪声，得到强调高频的 $z_T^{c,\prime}$
...
\text{Metric} & \text{Ours} & \text{StyleID} & \text{InstantStyle plus} & \text{StyleAlign} & \text{InstantStyle} & \text{DiffuseIT} & \text{InST} & \text{DiffStyle} \\ \hline
\text{ArtFID}\downarrow & \mathbf{21.499} & 28.801 & 25.886 & 36.269 & 37.524 & 40.721 & 40.633 & 41.464 \\
\text{FID}\downarrow & \mathbf{13.448} & 18.131 & 16.097 & 20.338 & 21.817 & 23.065 & 21.571 & 20.903 \\
\text{LPIPS}\downarrow & \mathbf{0.4881} & 0.5055 & 0.5140 & 0.6997 & 0.6446 & 0.6921 & 0.8002 & 0.8931 \\
\end{array}
$$
...
$$
\begin{array}{l|c|c|c}
\text{Configuration} & \text{ArtFID}\downarrow & \text{FID}\downarrow & \text{LPIPS}\downarrow \\ \hline
\text{Baseline} & 26.683 & 16.205 & 0.5509 \\
\text{+ FM}  & 24.112 & 15.103 & 0.4973 \\
\text{+ NG}  & 26.542 & 16.128 & 0.5496 \\
\text{StyleSSP} & \mathbf{21.499} & \mathbf{13.448} & \mathbf{0.4881} \\
\end{array}
$$

## 结论与思考

- 贡献：系统研究“采样起点”在训练自由风格迁移中的关键性，提出频率操作 + 反演负引导起点组件，增加两个可调参数 $\alpha$ 与 $\omega_i$
- 效果：在免训练的前提下同时提升 **内容保持与风格匹配**，避免风格内容泄漏
- 局限：
  - 依赖高质量基础模型（SDXL + IP-Instruct + ControlNet），推理成本较高
  - 频率比例 $\alpha$ 与负引导尺度 $\omega_i$ 仍需经验调参
